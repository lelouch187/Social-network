...
const UserDate: FC<ISerDateProps> = ({ userId, myId }) => {
  ...
  const changeMyStatus = async () => {
    await saveMyStatus(value)
      .unwrap()
      .then((res: any): any => (res.resultCode === 0 ? refetch() : alert('Произошла ошибка')));
    ...
    return (
      <div className="wrapper">
        <div className={s.wrapper}>
          {changeAvatarModal && <ModalAvatar setChangeAvatarModal={setChangeAvatarModal} />}
          {changeProfileModal && (
            <ModalProfile myProfile={data} setChangeAvatarModal={setChangeProfileModal} />
          )}
          <div className={s.wrapperImg}>
            <img src={data?.photos.large ? data.photos.large : ava} alt="аватар" />
            {Number(userId) === myId && (
              <p onClick={() => setChangeAvatarModal(true)} className={s.changeAvatar}>
                Изменить аватар
              </p>
            )}
          </div>
          <div className={s.userInfo}>
            <h3 className={s.title}>
              {data?.fullName}{' '}
              {Number(userId) === myId && (
                <span onClick={() => setChangeProfileModal(true)} className={s.titleChange}>
            ...
            <div>
              <p className={s.item}>
                <span>В поисках работы:</span> {data?.lookingForAJob ? 'да' : 'нет'}
              </p>
              <p className={s.item}>
                <span>Описание желаемой работы:</span>{' '}
                {data?.lookingForAJobDescription ? data?.lookingForAJobDescription : 'не заполнено'}
              </p>
            </div>
          </div>
        </div>
